...
final class LogInView: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        emailInputView.onContinueTapped = { [weak self] email in
                    self?.navigateToConfirmationView(with: email)
                }
    }
    ...
    private func navigateToConfirmationView(with email: String) {
            let confirmationVM = ConformationViewModel()
            confirmationVM.email = email 
            
            let confirmationVC = ConformationView(viewModel: confirmationVM)
